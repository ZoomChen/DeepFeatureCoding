...
rec_yuv_dir = '/abs_path/coding/classification/vgg16'
if not os.path.exists(rec_yuv_dir):
    os.makedirs(rec_yuv_dir)
...
def proc_thread(subdir_list, number_threads, process_id):
    ...
    for (idx,subdir) in enumerate(subdir_list):
        if (idx % number_threads == process_id):
            ...
            write_dir = os.path.join(rec_yuv_dir, subdir)
            if not os.path.exists(write_dir):
                os.makedirs(write_dir)
            for i in feat_list:
                exec_encode(op_dir, write_dir, i)
