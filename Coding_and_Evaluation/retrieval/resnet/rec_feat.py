...
def proc_dequant_yuv(yuv_path, maxLog2Val, pad_size, yuv_size, feat_format='NHWC'):
...
if __name__ == "__main__":
    ...
    for (idx,subdir) in enumerate(subdirs):
        ...
        for feat_type in feat_list:
            ...
            maxLog2Val, pad_size, yuv_size = pickle_load(meta_dir)
            for Qp in Qp_list:
                yuv_dir = os.path.join(read_dir, 'rec_{}_Qp{}.yuv'.format(feat_type, Qp))
                npy_dir = os.path.join(write_dir, '{}_Qp{}.npy'.format(feat_type, Qp))
                dequant_data = proc_dequant_yuv(yuv_dir, maxLog2Val, pad_size, yuv_size, 'NCHW')
