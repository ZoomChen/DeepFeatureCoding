...
class EmbedNetwork(nn.Module):
    def __init__(self, num_class, dims = 128, pretrained_base = True, *args, **kwargs):
    ...
    def forward(self, x, feat_type):
        assert feat_type in ['conv1', 'pool1', 'conv2', 'conv3', 'conv4', 'conv5']
        if feat_type=='conv1':
            ...
            pool5 = F.avg_pool2d(conv5, conv5.size()[2:])
            x_pool = pool5.contiguous().view(-1, 2048)
            x = self.fc_head(x_pool)
            x = self.embed(x)
...
if __name__ == "__main__":
    embed_net, fc = EmbedNetwork(num_class =13164, pretrained_base = True)
